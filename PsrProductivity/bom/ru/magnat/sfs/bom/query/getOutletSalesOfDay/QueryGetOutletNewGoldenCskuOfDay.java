...
public final class QueryGetOutletNewGoldenCskuOfDay extends
		QueryGeneric<QueryGetOutletNewGoldenCskuOfDayEntity> {
	...
	static final String query = "select 1 as Id, count(distinct i.ParentExt) as NewCount"
			...
			+ " inner join RegCskuState st on i.ParentExt = st.Csku and st.Abc = 1";

	public QueryGetOutletNewGoldenCskuOfDay(Context context, Date date,
			RefOutletEntity outlet) {
		super(context, QueryGetOutletNewGoldenCskuOfDayEntity.class, query);
		_day = date;
		_outlet = outlet;
