...
public final class QueryGetDistribution extends
		QueryGeneric<QueryGetDistributionEntity> {
	...
	static final String query = "SELECT 1 as Id,"
			...
			+ " coalesce(sum(Fact-FactCIS)/NULLIF(sum(Target),0),1)*100 as KpiIndex "
			+ " FROM RegKpiMatrix ";

	public QueryGetDistribution(Context context, Date date, int horizon,
			RefKpiEntity kpi) {
		super(context, QueryGetDistributionEntity.class, query);
